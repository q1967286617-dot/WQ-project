...
def summarize_backtest(
    ...
    three_way_comparison: Optional[Dict] = None,  # ← 新增
) -> Dict:
    out = {
    ...
    def _agg(col: str) -> pd.DataFrame:
        return (
            ...
            .agg(
                ...
                win_rate           = ("realized_holding_return",
...
def write_backtest_outputs(
    ...
    oracle_daily_df:  Optional[pd.DataFrame] = None,
    random_trades_df: Optional[pd.DataFrame] = None,
    oracle_trades_df: Optional[pd.DataFrame] = None,
) -> None:
    ensure_dir(out_dir)
    ...
    if oracle_daily_df is not None and not oracle_daily_df.empty:
        ensure_dir(ref_dir)
        _write_df(
            oracle_daily_df.drop(columns=["industry_exposure"], errors="ignore"),
            ref_dir / "oracle_ceiling_daily.csv",
        )
        if oracle_trades_df is not None and not oracle_trades_df.empty:
            _write_df(oracle_trades_df, ref_dir / "oracle_ceiling_trades.csv")
        _write_json(
            _return_metrics(oracle_daily_df["portfolio_ret"]),
            ref_dir / "oracle_ceiling_metrics.json",
        )
