...
def enrich_daily_report(daily_df: pd.DataFrame, benchmark_df: pd.DataFrame) -> pd.DataFrame:
    ...
    x["industry_exposure_json"] = x["industry_exposure"].apply(lambda v: json.dumps(v, ensure_ascii=True, sort_keys=True))
    return x
...
def summarize_backtest(
    ...
    out = {
        ...
        "signal_hit_rate_on_entries": float(pd.to_numeric(daily_df["signal_hit_rate"], errors="coerce").mean())
        if len(daily_df)
        else np.nan,
        ...
        "trades": {
            ...
            "win_rate": float((pd.to_numeric(trades_df.get("realized_holding_return"), errors="coerce") > 0).mean())
            if len(trades_df)
            else np.nan,
            "mean_trade_return": float(pd.to_numeric(trades_df.get("realized_holding_return"), errors="coerce").mean())
            if len(trades_df)
            else np.nan,
            "truncated_rate": float(pd.to_numeric(trades_df.get("truncated"), errors="coerce").mean())
            if len(trades_df)
            else np.nan,
        },
...
def build_trade_attribution(trades_df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    x = trades_df.copy()
    q = min(4, len(x))
    x["prob_bucket"] = pd.qcut(x["prob"].rank(method="first"), q=q, labels=[f"q{i}" for i in range(1, q + 1)]).astype("string")
...
def write_backtest_outputs(
    ...
    _write_df(daily_df.drop(columns=["industry_exposure"], errors="ignore"), out_dir / "daily_portfolio.csv")
