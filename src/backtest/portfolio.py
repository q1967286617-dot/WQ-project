...
def simulate_portfolio(
    ...
    row_lookup = {(pd.Timestamp(r["date"]), int(r["permno"])): r for _, r in x.iterrows()}
    ...
    for idx, dt in enumerate(dates):
        ...
        for row in planned_entries.get(dt, []):
            ...
            if idx <= cooldown_until.get(permno, -10**9):
                continue
            if (dt, permno) not in row_lookup:
                continue
            ...
            planned_end_idx = idx + holding_td - 1
        ...
        gross_ret = 0.0
        entry_cost = len(entries_today) * weight * cost_rate
        exit_positions = [p for p in active_today if p.active_end_idx == idx]
        exit_cost = len(exit_positions) * weight * cost_rate
        industry_exposure: Dict[str, float] = {}

        for pos in active_today:
            row = row_lookup.get((dt, pos.permno))
            stock_ret = float(row["DlyRet"]) if row is not None and pd.notna(row["DlyRet"]) else 0.0
            gross_ret += weight * stock_ret
        ...
        daily_ret = gross_ret - entry_cost - exit_cost
        daily_rows.append(
            {
                "date": dt,
                "portfolio_ret": daily_ret,
                "gross_ret": gross_ret,
            ...
            trade_rows.append(
                {
                    ...
                    "holding_days_realized": int(pos.active_end_idx - pos.entry_idx + 1),
                    ...
                    "realized_holding_return": pos.cum_mult - 1.0 - (2.0 * cost_rate),
                }
            )
