...
def main() -> None:
    ...
    logger.info(f"run_id={run_id}, C={args.C}, penalty={args.penalty}")
    ...
    logger.info(f"feature dim: num={len(num_cols)}, cat_ohe={X_train_cat.shape[1]}, total={len(num_cols)+X_train_cat.shape[1]}")
    ...
    X_val = np.hstack([val_df[num_cols].values, val_cat])
